...
// Tambahkan ini di bagian paling atas admin.js
loadDashboardStats();
loadPendingUsers();
loadApprovedUsers();
loadAllUsers();
...
async function loadDashboardStats() {
    try {
        ...
        document.getElementById('pendingModules').textContent = pendingModules;
        document.getElementById('pendingModulesCount').textContent = pendingModules;
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    document.getElementById('refreshModulesBtn').addEventListener('click', function() {
        loadPendingModules();
        loadApprovedModules();
        loadAllModules();
    });
    
    // Setup approve and reject module buttons in modal
    document.getElementById('approveModuleBtn').addEventListener('click', function() {
        const moduleId = this.getAttribute('data-module-id');
        approveModule(moduleId);
    });
    
    document.getElementById('rejectModuleBtn').addEventListener('click', function() {
        const moduleId = this.getAttribute('data-module-id');
        rejectModule(moduleId);
    });
    ...
    if (settingsForm) {
        ...
        settingsForm.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                const autoApproveUsers = document.getElementById('autoApproveUsers').checked;
